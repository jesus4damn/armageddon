...
const MeteorPage: React.FC<any> = () => {
    const history = useHistory()
    const { id } = useParams<{id: string}>()
    const [loading, setLoading] = useState<boolean>(true)
    const [meteorData, setMeteorData] = useState<IMeteor | null>(null)
    ...
    const getMeteor = async () => {
        try{
            setLoading(true)
            const response = await API.get(`https://api.nasa.gov/neo/rest/v1/neo/${id}?api_key=E9pMitzwMFuDNh4ElHuYpfoq8IMCBjsYEcsBsydh`)
    ...
    return (
        <>
            ...
            error ? <div style={{textAlign:'center', width: '100%'}}>
                        <Error>404</Error>
                        <Link onClick={() => history.push('/')}>Вернуться к списку астероидов</Link>
                    </div> :
